...
NOTE: The static handler considers that the local filesystem
is trusted. Be careful when writing data on disk to files
or directories served by the static handler.

NOTE: The static handler follows symlinks and symlinks may
point to an entirely different location than configured.
For example, when configured with `dir`, the static handler
may serve files from outside of this directory if the
