...
Prevent sending HTTP responses with invalid response headers.
The default `error_terminate` will result in an HTTP 500
response sent when invalid response headers are detected,
except when the invalid headers are trailer headers: in that
case the connection is dropped. When set to `ignore` the
check is not performed at all: use at your own risk.
...
Maximum length of the authority component of the target URI.
...
* *2.17*: The `max_authority_length` option was added.
* *2.16*: The `invalid_response_headers` option was added.
