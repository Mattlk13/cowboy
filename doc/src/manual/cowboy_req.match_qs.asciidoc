...
match_qs(Fields :: cowboy:fields(), Req :: cowboy_req:req())
    -> match_qs(Fields, Req, #{})
match_qs(Fields :: cowboy:fields(), Req :: cowboy_req:req(), Opts)
    -> #{atom() => any()}
...
The `max_keys` option limits the maximum number of
keys in the query string. It defaults to 100.
...
* *2.17*: Function `match_qs/3` introduced with a `max_keys` option.
